...
import { buildFontEmbedCSS, collectFontFamilies } from '@h5design/core';
import {
  ...
  try {
    return await withTimeout(
      toCanvas(host, {
        ...
        // 必须自带 @font-face：截帧同样是「DOM → SVG 当图片渲染」，页面里 FontFace 注册的
        // 字体在 SVG 中不可见，不内嵌就会让视频里的文字全部回退系统字体。
        // 同时 skipFonts 跳过 html-to-image 自带的抓取（只扫 styleSheets，扫不到且会 reject）。
        skipFonts: true,
        fontEmbedCSS,
      }),
      RENDER_TIMEOUT_MS,
...
export async function exportProjectToVideo(project: Project): Promise<void> {
  ...
  // 1.5) 内嵌自定义字体（@font-face + base64 data URL）——截帧走的是「DOM → SVG 当图片渲染」，
  // 页面里 FontFace 注册的字体在 SVG 内不可见，不内嵌则视频里所有文字回退系统字体。
  let fontEmbedCSS = '';
  try {
    fontEmbedCSS = await buildFontEmbedCSS(collectFontFamilies(project));
  } catch {
    /* 内嵌失败不阻断导出（文字会回退系统字体） */
  }
  ...
  try {
    ...
    // 立刻画一帧，避免开头长时间空白
    lastFrame = await captureFrame(current.host, width, height, fontEmbedCSS);
    ...
    for (let i = 0; i < pageCount; i++) {
      ...
      while (true) {
        ...
        if (t >= duration) break;
        const frame = await captureFrame(current.host, width, height, fontEmbedCSS);
        lastFrame = frame;
      ...
      if (i < pageCount - 1) {
        ...
        const nextRestFrame = await captureFrame(nextRest.host, width, height, fontEmbedCSS);
