...
export function createVideoAudioStream(
  ...
  // 关键修复：必须把音轨直接 addTrack 到 captureStream 自身产生的 MediaStream 上，
  // 而不是用 new MediaStream([...videoTracks, aTrack]) 重组——后者在 Chrome/Firefox 下
  // MediaRecorder 经常「吞掉」来自 AudioContext(MediaStreamDestinationNode) 的独立音轨，
  // 表现为画布正常录制、背景音乐却静默（即「预览有声、导出无声」）。
  if (typeof videoStream.addTrack === 'function') {
    videoStream.addTrack(aTrack);
  } else {
