...
const ZOOM_MIN = 0.1;
const ZOOM_MAX = 8;
/** 滚轮灵敏度：scaleFactor = e^(-deltaY × 灵敏度)，一次标准滚轮（deltaY=100）约缩放 1.2 倍 */
const ZOOM_WHEEL_SENSITIVITY = 0.0018;
/** 按钮单步缩放倍率 */
const ZOOM_BUTTON_STEP = 1.25;
/** 平移时至少保证画布有这么多的像素留在桌面区域内，避免把画布「推丢」 */
const ZOOM_KEEP_VISIBLE = 56;
/** 标尺刻度步长候选（画布坐标），按缩放自动挑选，保证屏幕上主刻度间距不小于 ~90px */
const RULER_STEPS = [5, 10, 20, 25, 50, 100, 200, 250, 500, 1000];

const clampZoom = (z: number) => Math.min(ZOOM_MAX, Math.max(ZOOM_MIN, z));

/** 平移边界：内容（标尺+画布）至少保留 ZOOM_KEEP_VISIBLE 像素可见 */
function clampPan(
  p: { x: number; y: number },
  zoom: number,
  deskW: number,
  ...
  const minX = ZOOM_KEEP_VISIBLE - w;
  const maxX = deskW - ZOOM_KEEP_VISIBLE;
  const minY = ZOOM_KEEP_VISIBLE - h;
  const maxY = deskH - ZOOM_KEEP_VISIBLE;
  return {
    x: Math.round(minX > maxX ? (deskW - w) / 2 : Math.min(maxX, Math.max(minX, p.x))),
    y: Math.round(minY > maxY ? (deskH - h) / 2 : Math.min(maxY, Math.max(minY, p.y))),
  };
}
...
function Ruler({
  ...
  // 刻度步长随缩放自适应：放大时步长变细、缩小时变粗，屏幕上密度保持恒定
  const majorStep = RULER_STEPS.find((s) => s * zoom >= 90) ?? RULER_STEPS[RULER_STEPS.length - 1];
  const minorStep = majorStep / 2;
  if (viewport <= 0) return null;
  // 当前可见的画布坐标范围（屏幕边缘 → 画布坐标）
  const cMin = -pan / zoom;
  const cMax = (viewport - pan) / zoom;
  const start = Math.floor(cMin / minorStep) * minorStep;
  const ticks: React.ReactNode[] = [];
  for (let c = start; c <= cMax + 1e-6; c += minorStep) {
    const screen = pan + c * zoom; // 在视口坐标系中的位置
    if (screen < -2 || screen > viewport + 2) continue;
    const idx = Math.round(c / minorStep);
    const major = idx % 2 === 0 && Number.isInteger(c);
    if (isH) {
      ticks.push(
        <line
          key={`t-${c}`}
          x1={screen}
          y1={major ? RULER - 11 : RULER - 5}
          x2={screen}
          y2={RULER}
          stroke="#9ca3af"
          strokeWidth={1}
      ...
      if (major && c > -1e-6) {
        ticks.push(
          <text key={`l-${c}`} x={screen} y={RULER - 13} fontSize={9} fill="#6b7280" textAnchor="middle">
            {Math.round(c)}
          </text>,
        );
      }
      ...
      if (major && c > -1e-6) {
        ticks.push(
          <text key={`l-${c}`} x={RULER - 4} y={screen + 3} fontSize={9} fill="#6b7280" textAnchor="end">
            {Math.round(c)}
          </text>,
        );
      }
  ...
  return (
    <svg width={isH ? viewport : RULER} height={isH ? RULER : viewport} className="block">
      {ticks}
    </svg>
  );
}
...
function getTargetGuides(
  ...
  const vertical: number[] = [0, canvasW / 2, canvasW];
...
function snapCalc(
  ...
  const targets = getTargetGuides(otherElements, canvasH, canvasW);
  const guides: Guide[] = [];
...
interface RenderProps {
  ...
  /** 字体加载纪元：每完成一次 FontFace 加载 +1，拼进文本节点 key 强制 Konva 重建重测折行 */
  fontEpoch: number;
}
...
function renderElement({
  ...
  switch (el.type) {
    case 'text': {
      ...
      // 文本轮廓：先描边、后填充。Konva 默认是 fill→stroke，描边会压在填充之上，
      // 字形相接处的边线因此外露（阿拉伯文等连写文字最明显）→ 反转后只保留外框轮廓。
      const hasTextOutline = (textEl.outlineWidth || 0) > 0 && !!textEl.outlineColor;
      // 阿拉伯文等连写文字：字间距改用延长符（kashida）展开，保持笔画相连，
      // 而非用 letterSpacing 在两字母间插间隙把连写拆开。编辑器与发布页共用同一函数。
      const kashida = kashidaForLetterSpacing(textEl.text, textEl.letterSpacing ?? 0, textEl.fontSize ?? 16, true);
      // 阴影不再挂在「描边+填充」同一节点上：那样 Konva 会在描边后投一次阴影、填充后又投一次，
      // 第二次阴影会盖掉一部分轮廓（表现为轮廓时有时无）。改为独立的最底层阴影剪影层，
      // 见下方 <Text fill={shadowColor}> 那一层；此处 textShadow 仅用于背景矩形。
      const textShadow = hasRealShadow(textEl)
        ? {
            ...
            shadowForStrokeEnabled: hasTextOutline,
          }
        : {};
      ...
      return (
        ...
        >
          ...
          {/* ① 阴影剪影层（最底）：字形 + 轮廓同色加粗 = 贴纸整体剪影，再整体偏移并投模糊阴影。
              必须独立成层且画在所有内容之下，否则会盖住轮廓（问题根因）。 */}
          ...
          {/* ② 轮廓层：只描边不填充 → 外框宽度恒为 outlineWidth，不会被上层填充/阴影覆盖，
              连写文字连接处也不会出现内部竖线 */}
          {hasTextOutline && (
            <Text
...
interface PageBackgroundImageProps {
  ...
  /** 背景图透明度 0~1（默认 1），只作用于图片本身，不影响背景色 */
  opacity?: number;
}
...
export default function EditorCanvas({ onPreview, onSave, onSettings, isSaving }: EditorCanvasProps) {
  ...
  // 字体加载纪元：字体异步加载完成后 +1，驱动文本节点 key 变化以强制 Konva 重测折行
  ...
  // 同时作为 commitTextEdit 的幂等守卫，防止 blur 与点击空白重复提交。
  ...
  // 右键上下文菜单位置（视口坐标）；为 null 时不渲染菜单
  const [menuPos, setMenuPos] = useState<{ x: number; y: number } | null>(null);

  // 画布右键菜单：根据光标下的元素调整选择并弹出菜单
  const onCanvasContextMenu = (e: React.MouseEvent) => {
    e.preventDefault();
    const modifier = e.ctrlKey || e.metaKey || e.shiftKey;
    if (hoveredId) {
      // 命中元素：若不在当前多选集合内则选中它（按住修饰键则切换）
      if (!selectedIds.includes(hoveredId)) {
        select(hoveredId, modifier ? { toggle: true } : undefined);
      }
    } else {
      // 命中空白：清空选择，仅保留"粘贴"
      clearSelection();
  ...
  const canvasW = project.width ?? CANVAS_DEFAULT.width;
  const canvasH = page.height ?? CANVAS_DEFAULT.height;
  const W0 = canvasW + RULER;
  const H0 = canvasH + RULER;
  ...
  // 舞台尺寸 = 视口尺寸（铺满桌面区域），随窗口尺寸变化；缩放/平移由 Konva 内部承担
  const [stageSize, setStageSize] = useState<{ w: number; h: number }>({ w: 0, h: 0 });
  // 测量视口真实尺寸并驱动 Konva 舞台铺满；缩放/平移由 Konva 内部 scale/position 承担，矢量内容始终按当前分辨率重绘（无损）
  useLayoutEffect(() => {
  ...
  // 用 ref 镜像最新 zoom/pan，供原生 wheel 监听读取，避免闭包拿到旧值
  ...
  const zoomAroundCenter = useCallback(
    (next: number) => {
      ...
      const np = clampPan({ x: cx - wx * nz, y: cy - wy * nz }, nz, rect.width, rect.height, W0, H0);
      setZoom(nz);
      setPan(np);
    },
    [W0, H0],
  ...
  // 适配屏幕：取能完整显示画布的最大缩放（不超过 100%），并居中
  const fitView = useCallback(() => {
    const vp = viewportRef.current;
    if (!vp) return;
    const rect = vp.getBoundingClientRect();
    if (rect.width < 2 || rect.height < 2) return;
    const z = clampZoom(Math.min(rect.width / W0, rect.height / H0, 1));
    const cx = (rect.width - W0 * z) / 2;
    const cy = (rect.height - H0 * z) / 2;
    const np = clampPan({ x: cx, y: cy }, z, rect.width, rect.height, W0, H0);
    setZoom(z);
    setPan(np);
  }, [W0, H0]);
  ...
  // 滚轮缩放：以指针位置为锚点（原生非被动监听，确保可 preventDefault 阻止页面滚动）
  useEffect(() => {
    const vp = viewportRef.current;
    if (!vp) return;
    const onWheel = (e: WheelEvent) => {
      ...
      const old = zoomRef.current;
      // 一次标准滚轮 deltaY≈100，factor≈e^(-0.18)≈0.835 → 缩小；反向放大
      const factor = Math.exp(-e.deltaY * ZOOM_WHEEL_SENSITIVITY);
      const nz = clampZoom(old * factor);
      if (nz === old) return;
      const wx = (px - panRef.current.x) / old;
      const wy = (py - panRef.current.y) / old;
      const np = clampPan({ x: px - wx * nz, y: py - wy * nz }, nz, rect.width, rect.height, W0, H0);
      setZoom(nz);
      setPan(np);
    };
  ...
  // 平移模式时让 Konva 画布不接收指针事件，避免误拖动元素 / 误选
  useEffect(() => {
    const c = stageRef.current?.container();
    if (c) c.style.pointerEvents = spaceDown ? 'none' : '';
  }, [spaceDown]);
  ...
  const onViewportMouseDown = (e: React.MouseEvent) => {
    ...
    panDragRef.current = start;
    setPanning(true);
    const onMove = (ev: MouseEvent) => {
      const d = panDragRef.current;
      if (!d) return;
      const nx = d.px + (ev.clientX - d.x);
      const ny = d.py + (ev.clientY - d.y);
      const vp = viewportRef.current;
      const rect = vp ? vp.getBoundingClientRect() : null;
      const np = rect
        ? clampPan({ x: nx, y: ny }, zoomRef.current, rect.width, rect.height, W0, H0)
        : { x: nx, y: ny };
      setPan(np);
    };
    const onUp = () => {
  ...
  useEffect(() => {
    ...
    // 仅以 canvasH 为依赖：窗口/画布高度变化才需要重新吸附默认位置；
    // tbPos 放入依赖会在每次拖拽后被置为非 null 又触发本 effect（guard 直接 return），
    // 属于无意义的重复执行。tbPos 的当前值由闭包捕获即可，无需进依赖。
  }, [canvasH]);
  ...
  const handleDragMove = useCallback(
    (e: Konva.KonvaEventObject<DragEvent>) => {
      ...
      const { snappedX, snappedY, guides: newGuides } = snapCalc(tempEl, others, canvasH, canvasW);
  ...
  const handleStageMouseDown = (e: Konva.KonvaEventObject<MouseEvent | TouchEvent>) => {
    ...
    // 不能直接 setEditingId(null)——那会让 textarea 同步卸载，blur 不再触发，编辑内容丢失
    commitTextEdit();
    const stage = stageRef.current;
    if (!stage) return;
  ...
  const handleStageMouseMove = () => {
    if (spaceDownRef.current) return; // 平移模式不更新框选
  ...
  const commitTextEdit = useCallback(() => {
    ...
    if (!id) return; // 幂等守卫：blur 与点击空白/mousedown 路径可能先后触发，只提交一次
    editingIdRef.current = null;
    pushHistory();
    updateElement(id, { text: editValue } as Partial<Element>);
    setEditingId(null);
  }, [editValue, pushHistory, updateElement]);
  ...
  return (
    ...
    >
      {/* Konva 舞台：铺满视口；缩放/平移由 Konva 内部 scale/position 承担，矢量内容无损重绘 */}
      <div ref={containerRef} className="absolute inset-0">
        ...
        <div className="pointer-events-none absolute left-0 top-0 right-0 z-20 overflow-hidden border-b border-gray-300 bg-gray-100" style={{ height: RULER }}>
          <Ruler orientation="h" zoom={zoom} pan={pan.x} viewport={stageSize.w} />
        </div>
        <div className="pointer-events-none absolute left-0 top-0 bottom-0 z-20 overflow-hidden border-r border-gray-300 bg-gray-100" style={{ width: RULER }}>
          <Ruler orientation="v" zoom={zoom} pan={pan.y} viewport={stageSize.h} />
        </div>
        <div className="pointer-events-none absolute left-0 top-0 z-30 border-b border-r border-gray-300 bg-gray-300" style={{ width: RULER, height: RULER }} />

        {/* DOM 覆盖层：与画布共用 pan+scale 映射，保证文本编辑框 / 图集预览与画布像素对齐 */}
        <div
          ...
          {editingEl && (
            <textarea
              ...
              style={{
                ...
                textAlign: toKonvaAlign(editingEl.align),
        ...
        {/* 缩放控制条：固定在视口右下角，不参与画布缩放；滚轮缩放 / 空格+拖拽平移 */}
        <div
          className="absolute bottom-4 right-4 z-20 flex items-center gap-1 rounded-lg border border-gray-200 bg-white/95 px-1 py-1 shadow-md backdrop-blur"
          onWheel={(e) => e.stopPropagation()}
          onMouseDown={(e) => e.stopPropagation()}
        >
          <button
            type="button"
            onClick={() => zoomAroundCenter(zoom / ZOOM_BUTTON_STEP)}
            title={t('editor:zoom.out')}
            className="flex h-7 w-7 items-center justify-center rounded text-gray-600 transition hover:bg-gray-100"
          >
          ...
          <button
            ...
            className="min-w-[52px] rounded px-1 text-center text-xs font-medium tabular-nums text-gray-700 transition hover:bg-gray-100"
          >
            {Math.round(zoom * 100)}%
          </button>
          <button
          ...
          <button
            ...
            className="flex h-7 items-center gap-1 rounded px-2 text-xs text-gray-600 transition hover:bg-gray-100"
          >
            <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
