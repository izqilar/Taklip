...
interface PublishModalProps {
  ...
  /**
   * 仅导出模式（终端用户）：隐藏「立即发布」页签，只保留 导出图片 / 导出视频(GIF)。
   * 终端用户没有发布作品（发布为模板 / 生成线上访问链接）的权限，只能导出本地文件。
   * 运营端（服务商 / 代理商 / 管理员）不传，保持三页签完整能力。
   */
  exportOnly?: boolean;
}
...
export default function PublishModal({
  ...
  // 选项卡状态（仅导出模式下没有「立即发布」，默认落在「导出图片」）
  const [activeTab, setActiveTab] = useState<TabKey>(exportOnly ? 'exportImage' : 'publish');
  // 宿主切换模式后校正：避免出现停在已隐藏页签上的空白面板
  useEffect(() => {
    if (exportOnly && activeTab === 'publish') setActiveTab('exportImage');
  }, [exportOnly, activeTab]);
  ...
  const handlePublish = useCallback(async () => {
    ...
    try {
      ...
      const res = await services.publish(projectId);
      setResult(res);
    } catch (err) {
  ...
  // 优先使用宿主注入的 url（web=发布 Project 返回 /p/xxx；运营端=模板画布跳转链接）；
  // 仅当 url 缺失时回退到内核默认的 /p/{publishCode}（保持历史 web 行为不变）。
  const fullUrl = result
    ? result.url
      ? result.url.startsWith('http')
        ? result.url
        : `${window.location.origin}${result.url}`
      : `${window.location.origin}/p/${result.publishCode}`
    : '';
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-gray-200 px-5 py-2.5">
          <h2 className="text-[14.5px] font-bold text-gray-800">
            {exportOnly ? t('publish:exportDialogTitle') : t('publish:publishSettingsTitle')}
          </h2>
          <button
            onClick={onClose}
