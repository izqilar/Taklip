...
interface Props {
  ...
  /** 图片元素（图片对象裁剪场景） */
  el?: ImageElement;
  /** 直接指定图片源：用于非图片元素场景（如页面背景图裁剪） */
  src?: string;
  /** 图片原始宽（不传则回退 el.naturalWidth / el.width） */
  naturalWidth?: number;
  /** 图片原始高（不传则回退 el.naturalHeight / el.height） */
  naturalHeight?: number;
  /** 隐藏「形状」选择区（页面背景只需矩形裁剪） */
  hideShapes?: boolean;
...
export default function ImageCropDialog({
  ...
  // 图片源与原始尺寸：优先用显式传入的 src/尺寸（背景图场景），否则取图片元素
  const imgSrc = src ?? el?.src ?? '';
  const natW = naturalWidth ?? el?.naturalWidth ?? el?.width ?? 1;
  const natH = naturalHeight ?? el?.naturalHeight ?? el?.height ?? 1;
