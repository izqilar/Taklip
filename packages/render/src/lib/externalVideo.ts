...
function buildFallbackIframe(url: string): string {
  return `<iframe src="${escapeAttr(
    url,
  )}" frameborder="0" allowfullscreen allow="autoplay; encrypted-media; fullscreen; picture-in-picture" scrolling="no" style="width:100%;height:100%;border:0;display:block;"></iframe>`;
}
