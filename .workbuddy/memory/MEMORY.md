...
- **P0+P1+P2+P3 已落地（2026-09-22）**：P1 员工登录与鉴权门控全通；P2 岗位字典统一到 `packages/core/src/staff-roles.ts` 单一真值源、删 `team-role.meta.ts` 与 `ProviderTeamMember` 旧表；**P3 员工操作审计留痕**——`AuditModule` 接入 `StaffModule`（覆盖 PROVIDER/AGENT/CONSOLE），`staff.service` 的 `create/update/remove/bindUser` 加 `actor: JwtUser` 首参并写 `AuditLog`（`action=STAFF_CREATE/UPDATE/REMOVE/BIND`，`targetType=ORG_STAFF`），三台 controller 传入 `req.user`。**红线 R-05/R-06 已生效**。P3 验证 `verify-staff-p3.mjs`(11/11)；回归 P1 16/16、P2 13/13、API 20/20 全绿。**留待做**：账户详情页「员工操作时间线」可视化 UI（数据层已就绪）。详见文档 §13.7。
