...
- **P0+P1+P2+P3 已落地（2026-09-22）**：P1 员工登录与鉴权门控全通；P2 岗位字典统一到 `packages/core/src/staff-roles.ts` 单一真值源、删 `team-role.meta.ts` 与 `ProviderTeamMember` 旧表；**P3 员工操作审计留痕 + 可视化**——`AuditModule` 接入 `StaffModule`（覆盖 PROVIDER/AGENT/CONSOLE），`staff.service` 的 `create/update/remove/bindUser` 加 `actor: JwtUser` 首参并写 `AuditLog`（`action=STAFF_CREATE/UPDATE/REMOVE/BIND`，`targetType=ORG_STAFF`），三台 controller 传入 `req.user`。**可视化**：新增按成员隔离端点 `GET {provider|agent|admin}/team/:id/audit-logs`（受 `OrgAccess`+`team:manage` 保护，由 `StaffService.listAudit(id)` 实现），前端 `staffTeamPages.tsx` 的 `StaffTeamMember` 详情页加 `StaffAuditTimeline` 组件展示时间线（动作胶囊+角色+时间+before/after diff，403 静默降级）。**红线 R-05/R-06 已生效**。验证 `verify-staff-p3.mjs`(11/11)+`verify-staff-p3-ui.mjs`(8/8)；回归 P1 16/16、P2 13/13、API 20/20 全绿。详见文档 §13.7。
