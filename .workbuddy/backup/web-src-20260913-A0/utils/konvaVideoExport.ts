...
import { preloadImages, cloneAndInline } from '@/utils/exportVideo';
import {
...
function buildPageChildren(
  ...
  for (const el of sorted) {
    children.push(createElement(KonvaElement, { key: el.id, el, images, scaleK }));
  }
  return children;
}
...
function ExportStage({ project, pageIndex, images, scaleK, stageRef, onRendered }: ExportStageProps) {
  ...
  const children = useMemo<ReactElement[]>(
    () => buildPageChildren(width, height, page, images, scaleK),
    [width, height, page, images, scaleK],
  );
